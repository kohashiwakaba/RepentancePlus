...
if true then
	...
	EID:addCollectible(CustomCollectibles.BLOOD_VESSELS[1], "Taking damage doesn't actually hurt the player, instead filling the blood vessel #This can be repeated 6 times until the vessel is full #Once it's full, using it or taking damage will empty it and deal 3 and 3.5 hearts of damage to the player, respectively #{{Warning}} The item cannot be easily swapped if it's not empty!")
	...
	EID:addCollectible(CustomCollectibles.RED_KING, "After defeating a boss, red crawlspace will appear in a middle of a room #{{Boss}} Entering the crawlspace brings you to another bossfight of high difficulty #Victory rewards you two items from Ultra secret room pool to choose from")
	...
	EID:addCollectible(CustomCollectibles.HANDICAPPED_PLACARD, "Places a handicapped placard on the ground #{{Weakness}} Every enemy inside the placard's area of effect is weakened and spawns bone spurs on death #The area becomes bigger the more damage you take in a room")
	...
	EID:addCard(CustomConsumables.SACRIFICIAL_BLOOD, "{{ArrowUp}} Gives +1.25 DMG up that depletes over the span of 25 seconds #{{Heart}} Heals you for one red heart if you have {{Collectible216}}Ceremonial Robes")
	...
	--
	EID:addCard(CustomConsumables.CROWN_OF_GREED , "{{Coin}} Spawns 1-2 golden pennies and grants -1 luck for each penny spawned")
...
if true then
	...
	EID:addCollectible(CustomCollectibles.MARK_OF_CAIN,"给予你一个以诺跟班 #死亡后，如果你有以诺以外的其他跟班，则献祭它们并复活你，同时以诺会变得更强 #复活后，获得短暂无敌时间，保留原有的{{Heart}}心之容器 #{{Warning}}一次性","该隐的印记","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.CHERUBIM,"发射神性眼泪的跟班","基路伯","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.ENRAGED_SOUL,"双击射击按钮会向你射击的方向发射一个亡魂 #亡魂会锁定它碰撞的第一个敌人，持续7秒造成伤害或者直到该敌人被杀死 #亡魂每次的攻击伤害从7开始，逐层递增 #亡魂可以锁定Boss #{{Warning}} 亡魂生成有7秒的冷却时间","愤怒亡魂","zh_cn")
	EID:addCollectible(CustomCollectibles.CEREMONIAL_BLADE,"#射击时，有7%几率发射一把穿透匕首，匕首不造成伤害，但会对敌人造成流血效果 #所有带有流血效果死亡的敌人都会掉落\"仪式用血\"，仪式用血使用后获得暂时的攻击加成","仪式匕首","zh_cn")
	EID:addCollectible(CustomCollectibles.CEILING_WITH_THE_STARS,"在每层开始或者在床上睡觉后给予你两个被动道具火焰跟班（所罗门魔典）","天花板上的星星","zh_cn")
	EID:addCollectible(CustomCollectibles.QUASAR,"使用后，摧毁所有底座道具 #每摧毁一个底座道具生成三个被动道具火焰跟班（所罗门魔典）","类星体","zh_cn")
	EID:addCollectible(CustomCollectibles.TWO_PLUS_ONE,"当前楼层每购买两个商品，第三个商品将只需要1块钱 #在同一个房间进行两次心交易，会使得其他道具免费","2+1","zh_cn")
	EID:addCollectible(CustomCollectibles.RED_MAP,"在所有后续楼层显示红隐藏房间的位置 #将饰品留在宝箱房或Boss房进出房间后饰品会转化为红钥匙碎片","红地图","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.DNA_REDACTOR,"药丸基于不同的颜色具有额外效果 #效果过多，请自行查询wiki","DNA解码器","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.RED_BOMBER,"+5炸弹 #免疫爆炸 #允许你投掷炸弹而不是直接在地板上放置炸弹","红炸弹","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.CAT_IN_A_BOX,"视线之外的敌人（由射击方向决定）会瘫痪，不会以任何方式攻击你，同时它们也不会受到伤害 #无法对Boss和小Boss生效","薛定谔的猫","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.SCALPEL,"消耗一个心之容器，若没有，则消耗两颗蓝心/黑心，让你永久地向一个随机方向发射眼泪 #这些眼泪造成75%的伤害，并拥有你的眼泪特效","解剖刀","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.NERVE_PINCH,"向一个方向射击或移动8秒会触发神经压迫 #{{ArrowDown}} 你会受到假伤害（钝刀片）并永久降低" .. tostring(CustomStatups.Speed.NERVE_PINCH) .. "速度 #有80%的几率免费触发一次主动道具 #一次性和无限使用的主动道具不会触发","神经箍缩","zh_cn")
	EID:addCollectible(CustomCollectibles.BLOOD_VESSELS[1],"受到伤害后会填充血皿,并免疫本次伤害 #血皿一共可以填充6次 #血皿填满之后,使用它或下次受到伤害会清空血皿并分别对玩家造成3或3.5颗心的伤害","血皿","zh_cn")
	...
	EID:addCollectible(CustomCollectibles.STARGAZERS_HAT, "生成一个观星乞丐，每次喂7块钱，会给予星象房道具 #只能用魂心充能 #充满能需要1个魂心 #{{Warning}}每层只能使用一次", "观星帽", "zh_cn")
	EID:addCollectible(CustomCollectibles.BOTTOMLESS_BAG, "使用后，将袋子悬空 #4秒内，玩家无法射击，并将周围所有敌方眼泪吸入袋中 #4秒后，获得短暂的无敌状态，并所有吸入的眼泪射出并追踪敌人", "无底袋", "zh_cn")
	...
	EID:addCollectible(CustomCollectibles.SOUL_BOND, "用星界锁链将自己锁在随机敌人身上并将其禁锢 #锁链会对敌人造成接触伤害 #离被锁链锁住的敌人太远会导致锁链断掉 #被锁链锁住的敌人击杀后有33%的几率掉落魂心", "灵魂锁链", "zh_cn")
	EID:addCollectible(CustomCollectibles.ANGELS_WINGS, "{{ArrowUp}} +0.3弹速 #角色的眼泪被穿透羽毛取代，距离越远，造成的伤害越高 #双击会发射类似教条的攻击方式之一（循环触发） #6秒冷却时间", "天使之翼", "zh_cn")
	...
	EID:addCollectible(CustomCollectibles.BOOK_OF_LEVIATHAN, "使用后，使房间内的所有敌人瘫痪 #瘫痪的敌人会失去速度，并在失去速度12秒后死亡 #每次使用消耗一把钥匙，如果没有钥匙则没有效果 #与钥匙相关的饰品有特殊的协同作用", "《利维坦》", "zh_cn")
	EID:addCollectible(CustomCollectibles.FRIENDLY_SACK, "每清理三个房间，生成一个弱跟班（如：蓝苍蝇，小屎等） #如果三个房间中包含Boss房，则会生成一个较强的跟班", "友好的麻袋", "zh_cn")	
	EID:addCollectible(CustomCollectibles.MAGIC_MARKER, "拾取后掉落一张卡牌 #使用后，变换当前卡牌（正位牌为卡牌编号+1，逆位牌为卡牌编号-1）", "魔法标记", "zh_cn")
	EID:addCollectible(CustomCollectibles.ULTRA_FLESH_KID, "追击敌人并造成接触伤害的跟班，类似于水蛭 #有3个阶段，每收集7.5颗红心进化一次", "究极食肉小子！", "zh_cn")
	...
	EID:addCollectible(CustomCollectibles.PURE_SOUL, "所有七宗罪Boss必定掉落各自的{{ColorRed}}罪恶道具{{CR}} #这个灵魂可以在隐藏房或者超级隐藏房发现，靠近会消失并随机生成一个七宗罪Boss #这个灵魂也会在通往七宗罪Boss房间的门口生成，以提醒你里面是哪个七宗罪Boss", "纯净灵魂", "zh_cn")
	EID:addCollectible(CustomCollectibles.HANDICAPPED_PLACARD, "在地上放置一个残疾人标语牌 #标语牌作用范围内的每个敌人都会被虚弱（卡牌力量?效果）并在死亡时产生骨刺 #你在房间内受到的伤害越高，标语牌范围越大", "残疾人标语牌", "zh_cn")
	...
	EID:addTrinket(CustomTrinkets.SLEIGHT_OF_HAND,"生成硬币时，每个硬币都有20%概率进行升级 #普通硬币->双倍硬币->粘币->五块钱硬币->十块钱硬币->幸运币->金金币","出千妙手","zh_cn")
	...
	EID:addTrinket(CustomTrinkets.EMPTY_PAGE,"在使用书时额外激活一个随机主动道具 #除了正常生成外，该饰品有33%概率在书房生成","空白的书页","zh_cn")
	...
	EID:addCard(CustomConsumables.NEEDLE_AND_THREAD,"移除一颗碎心并给予一个{{Heart}}心之容器", "针线包", "zh_cn")
	EID:addCard(CustomConsumables.QUEEN_OF_DIAMONDS,"生成1-12个随机{{Coin}}硬币", "方片Q", "zh_cn")
	EID:addCard(CustomConsumables.KING_OF_SPADES,"失去所有钥匙（但不超过33个）并生成与失去钥匙数量成比例概率的掉落物 #可生成饰品或道具，分别在第9和21个钥匙时达到100% #如果角色有金钥匙，也会被移除并显著提高失去钥匙的数量", "梅花K", "zh_cn")
	EID:addCard(CustomConsumables.KING_OF_CLUBS,"失去所有炸弹（但不超过33个）并生成与失去炸弹数量成比例概率的掉落物 #可生成饰品或道具，分别在第9和21颗炸弹时达到100% #如果角色有金炸弹，也会被移除并显著提高失去炸弹的数量", "黑桃K", "zh_cn")
	...
	EID:addCard(CustomConsumables.UNO_REVERSE_CARD,"一次性发光沙漏效果", "uno？", "zh_cn")
	EID:addCard(CustomConsumables.LOADED_DICE,"{{ArrowUp}} 当前房间{{ArrowDown}} +10幸运", "灌铅骰子", "zh_cn")
	...
	EID:addCard(CustomConsumables.SACRIFICIAL_BLOOD,"{{ArrowUp}} +1.25攻击力，并在25秒内持续衰减 #如果你有仪式长袍，则治疗一颗红心 #{{Warning}} 使用该物品越多，伤害衰减得越快", "仪式用血", "zh_cn")
	EID:addCard(CustomConsumables.LIBRARY_CARD,"随机给予一本书的效果", "借书证", "zh_cn")
	EID:addCard(CustomConsumables.FLY_PAPER,"生成8只环绕苍蝇 #苍蝇被弹幕打到会转化为蓝苍蝇", "粘蝇板", "zh_cn")
	...
	EID:addCard(CustomConsumables.DEMON_FORM,"每进入一个新的未清理的房间,你的伤害就会增加{{ArrowUp}} 0.2 #进入Boss房间时伤害增加{{ArrowUp}} 1 #进入新的楼层后增益消失", "恶魔形态", "zh_cn")
	...
	EID:addCard(CustomConsumables.CURSED_CARD, "使用后，当前房间原本应该受到的伤害都会转换为获得一颗碎心和永久的射速提升{{ArrowUp}}", "诅咒卡", "zh_cn")
                 
	EID:addPill(CustomPills.ESTROGEN,"只保留1颗红心，将其余红心扣除，不影响魂心/黑心", "雌激素上升", "zh_cn")
	...
	EID:addPill(CustomPills.YUCK,"生成1个腐心 #30秒内每捡起一颗红心都会产生蓝苍蝇", "呸！", "zh_cn")
	EID:addPill(CustomPills.YUM,"生成2-5个半红心 #30秒内每捡起一颗红心都会给予永久属性上升{{ArrowUp}}", "嗯！", "zh_cn")	
                
	EID:addCard(CustomConsumables.CROWN_OF_GREED , "使用后在当前房间生成1-2个金金币，但是每拾取一次金币幸运{{ArrowDown}}-1", "贪婪之冠", "zh_cn")
	...
	EID:addCard(CustomConsumables.VOID_OF_GLUTTONY, "在7秒内，如果你的红心不足，你会以每秒1.5红心的速度快速恢复它，同时每次恢复速度都会降低{{ArrowUp}} -0.01 #杀死敌人会使该效果的持续时间延长2秒", "暴食之虚", "zh_cn")
	...
	EID:addCard(CustomConsumables.MASK_OF_ENVY, "将所有心之容器转化为充满腐心的骨心", "嫉妒面具", "zh_cn")
	...
	EID:addEntity(5, 10, 98, "受缚者之心", "消耗1把钥匙（或拥有金钥匙）才能拾取 #拾取后，获得1颗蓝心并生成1个鬼魂 #鬼魂会与角色一同发射眼泪并阻挡敌人的眼泪 #鬼魂会在受到3次伤害后消失,同时掉落半颗魂心", "zh_cn")
